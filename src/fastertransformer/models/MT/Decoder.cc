...
#include "src/fastertransformer/models/MT/Decoder.h"

namespace fastertransformer {

template<typename T>
void MTDecoder<T>::initialize()
{
...
template<typename T>
void MTDecoder<T>::allocateBuffer()
{
...
template<typename T>
void MTDecoder<T>::allocateBuffer(size_t batch_size)
{
    if (is_allocate_buffer_ == false) {
        decoder_normed_input_ = reinterpret_cast<T*>(
...
template<typename T>
bool MTDecoder<T>::isValidBatchSize(size_t batch_size)
{
...
template<typename T>
MTDecoder<T>::MTDecoder(size_t           max_batch_size,
                    size_t           head_num,
...
template<typename T>
MTDecoder<T>::MTDecoder(MTDecoder<T> const& decoder):
...
template<typename T>
void MTDecoder<T>::forward(std::vector<Tensor>*                      output_tensors,
                         const std::vector<Tensor>*                input_tensors,
                         const std::vector<MTDecoderLayerWeight<T>>* decoder_layer_weight)
{
    // input tensors:
    ...
    for (uint l = 0; l < num_layer_; l++) {
        ...
        printf("[DEBUG] self attention forward\n" );
        ...
        printf("[DEBUG] invokeAddBiasResidualLayerNorm\n" );
        ...
        printf("[DEBUG] cross_attention_layer_->forward\n" );
        ...
        invokeAddBiasResidualLayerNorm(decoder_output,
                              ...
                              decoder_layer_weight->at(l).post_layernorm_weights.gamma,
                              decoder_layer_weight->at(l).post_layernorm_weights.beta,
                              layernorm_eps_,
                              batch_size,
                              hidden_units_,
                              stream_);
...
template class MTDecoder<float>;
template class MTDecoder<half>;
#ifdef ENABLE_BF16
template class MTDecoder<__nv_bfloat16>;
#endif
