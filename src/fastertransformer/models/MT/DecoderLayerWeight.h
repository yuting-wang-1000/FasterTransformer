...
template<typename T>
struct MTDecoderLayerWeight {

    MTDecoderLayerWeight() = default;
    MTDecoderLayerWeight(const int hidden_units, const int inter_size, const int mem_hidden_units):
    ...
    ~MTDecoderLayerWeight()
    {
        if (is_maintain_buffer == true) {
            ...
            post_layernorm_weights.beta            = nullptr;
            post_layernorm_weights.gamma           = nullptr;
            is_maintain_buffer                     = false;
        }
    }

    MTDecoderLayerWeight(const MTDecoderLayerWeight& other):
    ...
    MTDecoderLayerWeight& operator=(const MTDecoderLayerWeight& other)
    {
    ...
    LayerNormWeight<T> post_layernorm_weights;

private:
    void setWeightPtr()
    {
        post_layernorm_weights.beta            = weights_ptr[0];
        post_layernorm_weights.gamma           = weights_ptr[1];
