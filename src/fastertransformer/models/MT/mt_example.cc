...
template<typename T>
int mtdecodingExample(const size_t batch_size,
...
{
    if (argc != 14) {
        printf("[ERROR] mt_example batch_size beam_width head_num size_per_head inter_size vocab_size"
               " num_layers max_seq_len memory_max_seq_len memory_hidden_units top_k top_p data_type\n");
        printf("e.g., ./bin/mt_example 4 1 8 64 2048 30000 6 32 32 512 0 0.6 1\n");
        return 0;
    }
    ...
    return mtdecodingExample<float>(batch_size, beam_width, head_num, size_per_head, inter_size, vocab_size, num_layers, max_seq_len, memory_max_seq_len, memory_hidden_units, top_k, top_p);
}
                    ...
                    const float  top_p)
{
    ...
    fastertransformer::MTDecodingWeight<T> decoding_weights(
        hidden_units, inter_size, vocab_size, num_layers, max_seq_len, memory_hidden_units);
    MTDecoding<T> decoding = MTDecoding<T>(batch_size,
                                       max_seq_len,
                                       memory_max_seq_len,
                                       beam_width,
                                       head_num,
