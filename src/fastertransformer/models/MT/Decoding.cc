...
#include "src/fastertransformer/models/MT/Decoding.h"
#include "src/fastertransformer/models/MT/DecodingWeight.h"

namespace fastertransformer {

template<typename T>
void MTDecoding<T>::initialize()
{
    decoder_ = new MTDecoder<T>(max_batch_size_ * beam_width_,
                              head_num_,
                              size_per_head_,
                              inter_size_,
                              num_layer_,
...
template<typename T>
void MTDecoding<T>::allocateBuffer()
{
...
template<typename T>
bool MTDecoding<T>::isValidBatchSize(size_t batch_size)
{
...
template<typename T>
bool MTDecoding<T>::isValidSeqLen(size_t seq_len)
{
    if (max_seq_len_ == 0) {
...
template<typename T>
bool MTDecoding<T>::isValidMemSeqLen(size_t seq_len)
{
...
template<typename T>
MTDecoding<T>::MTDecoding(size_t           max_batch_size,
                      size_t           max_seq_len,
...
template<typename T>
MTDecoding<T>::MTDecoding(MTDecoding<T> const& decoding):
    BaseLayer(decoding),
...
template<typename T>
void MTDecoding<T>::forward(std::vector<Tensor>*       output_tensors,
                          const std::vector<Tensor>* input_tensors,
                          const MTDecodingWeight<T>*   decoding_weights)
{
    FT_LOG_DEBUG(__PRETTY_FUNCTION__);
    // input_tensors:
...
template<typename T>
void MTDecoding<T>::forward(std::unordered_map<std::string, Tensor>*       output_tensors,
...
template<typename T>
void MTDecoding<T>::forward(TensorMap*               output_tensors,
                          ...
                          const MTDecodingWeight<T>* decoding_weights)
{
    ...
    for (int step = 1; step < (int)max_seq_len_; step++) {
        ...
        decoder_output_tensors = decoder_input_tensors;
        sync_check_cuda_error();
...
template class MTDecoding<float>;
}  // namespace fastertransformer
