...
template<typename T>
class MTDecoder: public BaseLayer {
private:
    // buffer handling
...
public:
    MTDecoder(size_t           max_batch_size,
            size_t           head_num,
    ...
    MTDecoder(MTDecoder<T> const& decoder);

    ~MTDecoder();

    void forward(std::vector<Tensor>*                      output_tensors,
                 const std::vector<Tensor>*                input_tensors,
                 const std::vector<MTDecoderLayerWeight<T>>* decoder_layer_weights);
};
